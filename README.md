...
Инфраструктура звонков находится в `infra/livekit/`. LiveKit API secret хранится только на сервере и в Vercel; Android получает короткоживущий токен после проверки комнатного ключа через `/api/livekit-token`.
