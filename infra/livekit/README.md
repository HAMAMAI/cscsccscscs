...
Один узел LiveKit с аудио-only WebRTC, TURN/UDP на `443`, TURN/TLS на внешнем TCP `443` через SNI и WebSocket-сигналингом на том же TCP `443`. Встроенный token-service проверяет комнатный ключ через Supabase и выдаёт только короткоживущие токены с правом публикации микрофона.
